...
void *readtx(void *arg){
  ...
  start_operation(node->tid,node->count); //Lock all other operations in the transaction
  zgt_p(0); //Lock the TM table
  zgt_tx *tx = get_tx(node->tid);  //Get the current transaction
  tx->set_lock(node->tid,1,node->obno,node->count, 'S');  //Lock or wait for the resourse
  finish_operation(node->tid);  //Unlock all other operations in the transaction
  pthread_exit(NULL);
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  zgt_p(0);  //Lock the TM table
  zgt_tx *tx = get_tx(node->tid);  //Get the current transaction
  tx->set_lock(node->tid,1,node->obno,node->count, 'X');  //Lock or wait for the resourse
  finish_operation(node->tid);  //Unlock all other operations in the transaction
  pthread_exit(NULL);
} 
...
void *aborttx(void *arg)
{
  ...
  //write your code
  open_logfile_for_append();  //Open the log file for writing
  start_operation(node->tid,node->count);  //Lock all other operations in the transaction
  zgt_p(0);  //Lock the TM table
  zgt_tx *tx = get_tx(node->tid);  //Get the current transaction
  if(tx!=NULL){
    do_commit_abort(node->tid,'A');  //Abort Transaction
...
void *committx(void *arg)
{
 //remove the locks before committing
  ...
  zgt_tx *tx = get_tx(node->tid);  //Get the current transaction
  if(tx!=NULL){
    do_commit_abort(node->tid,'E');  //Commit Transaction
...
void *do_commit_abort(long t, char status){
  ...
  open_logfile_for_append();  //Open the log file for writing
  int i ,semnumber;
  zgt_tx *tx = get_tx(t);  //Get the current transaction
  tx->status = status;  //Get the status
  tx->free_locks();  //Unlock
  semnumber = tx->semno;
  if(semnumber > -1){
  tx->remove_tx();
  int num_waits;
  num_waits = zgt_nwait(tx->semno);
  for(i=num_waits;i>0;i--){
    zgt_v(tx->semno);  //Unlock waiting transaction
  }
 // zgt_v(tx->semno);
}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  //write your code
  open_logfile_for_append();  //Open log file for writing
  if(lockmode1=='X'){ //Check which lock mode is requied
  zgt_hlink *current,*txcur;
  int number;
  current=ZGT_Ht->findt(this->tid,sgno1,obno1);  //Find if the transaction holds the lock
  if(current!=NULL)
  { printf("I have lock %d\n",tid1);
    ...
    {
      txcur=ZGT_Ht->find(sgno1,obno1);  //Find the transaction which hold the lock
      if(txcur==NULL)  //If resource is not in the table
      {   printf("Adding into the table obnum %d\n",obno1);
          number=ZGT_Ht->add(this,sgno1,obno1,lockmode1);
          if(number>=0)
          {
            zgt_v(0);
            perform_readWrite(tid1,obno1,lockmode1);
          }else{
            //printf("Not able to insert into the hash");
            return 0;
          }
      }else  //Wait for the resourse
        {   printf("Waiting for the other transaction\n");
            this->status = TR_WAIT;
            this->lockmode = lockmode1;
            this->obno = obno1;
            this->setTx_semno(txcur->tid,txcur->tid);
            zgt_v(0);
            zgt_p(txcur->tid);
            this->status = TR_ACTIVE;
            zgt_p(0);
      ...
      zgt_hlink *current,*txcur;
      int number;
      current=ZGT_Ht->findt(this->tid,sgno1,obno1);
      if(current!=NULL)
      {
        this->status = TR_ACTIVE;
        this->lockmode=lockmode1;
      ...
      }else
        {
          ...
          if(txcur==NULL)
          {
            ...
            {   if(current->lockmode=='S'){  //If the resourse is locked in S then grant the lock
                    status = TR_ACTIVE;
                    txcur->lockmode=lockmode1;
                    zgt_v(0);
                    perform_readWrite(tid1, obno1, lockmode1);
                }else{
                this->status = TR_WAIT;
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  int temp;
  open_logfile_for_append();
  int obval=ZGT_Sh->objarray[obno]->value;
  if(lockmode=='X'){
    ZGT_Sh->objarray[obno]->value= obval + 1;
    ...
    fflush(logfile);
    for(temp=0; temp<=ZGT_Sh->optime[tid] ; temp++)
    {
